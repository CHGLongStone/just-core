...
class LightOpenID{
	...
	*/
	protected $server, $version, $trustRoot, $aliases, $identifier_select = false
			, $ax = false, $sreg = false, $data, $checkMode = 'checkid_setup'; 
	/**
	* static properties
	*/
	...
	*/
	protected function request($url, $method='GET', $params=array()){
		...
		case 'HEAD':
			...
			#echo __METHOD__.'::'.__LINE__.'<b>==$url['.$url.']=</b><br>';
			$url = $url . ($params ? '?' . $params : '');
			#echo __METHOD__.'::'.__LINE__.'<b>==$url['.$url.']=</b><br>';
	...
	*/
	public function __set($name, $value){
		...
		#echo __METHOD__.__LINE__.'---------\---\---/assd<br>';
		return;
	}
	...
	*/
	public function __get( $name ){
		#echo __METHOD__.'::'.__LINE__.'$name<b>'.$name.'</b><br><br>';
	...
	*/
	public function authUrl($identifier_select = null){
		...
		#echo __METHOD__.'::'.__LINE__.'<br>$this->server['.$this->server.']$this->identity['.$this->identity.']';//$this->discover($this->identity)['.$this->discover($this->identity).']';
	...
	public function validate(){
		...
		if ($this->data['openid_return_to'] != $this->returnUrl) {
			...
			#echo '$this->data["openid_return_to"]['.$this->data["openid_return_to"].']'.__FILE__.'::'.__LINE__.'<br>';
			#echo '$this->returnUrl['.$this->returnUrl.']'.__FILE__.'::'.__LINE__.'<br>';
			return false;
		}
		...
		#echo 'explode(",", $this->data["openid_signed"])<pre>'.var_export(explode(',', $this->data["openid_signed"]), true).'</pre>'.__FILE__.'::'.__LINE__.'<br>';
		#echo 'server<pre>'.var_export($server, true).'</pre>'.__FILE__.'::'.__LINE__.'<br>';
		#echo 'params<pre>'.var_export($params, true).'</pre>'.__FILE__.'::'.__LINE__.'<br>';
		#echo 'response<pre>'.var_export($response, true).'</pre>'.__FILE__.'::'.__LINE__.'<br>';
		#echo 'this->data<pre>'.var_export($this->data, true).'</pre>'.__FILE__.'::'.__LINE__.'<br>';
	...
	protected function getAxAttributes(){
		...
		foreach ($this->data as $key => $value) {
			...
			if (substr($key, 0, strlen($keyMatch)) != $keyMatch) {
			   # echo __METHOD__.'keyMatch==['.$keyMatch.']<br>';
				continue;
			}
			$key = substr($key, strlen($keyMatch));
			if (!isset($this->data['openid_' . $alias . '_type_' . $key])) {
			   # echo __METHOD__.'alias==['.$alias.']<br>';
		...
		#echo 'getAxAttributes=>$attributes<pre>'.var_export($attributes,true).'</pre>';
		return $attributes;
	}
	/**
	* DESCRIPTOR: 
	...
	*/
	protected function getSregAttributes(){
		...
		#echo 'getSregAttributes=>$attributes<pre>'.var_export($attributes,true).'</pre>';
		return $attributes;
	}
	...
	public function discover($url){
		...
		for ($i = 0; $i < 5; $i ++) {
			...
			if (!$server) {
				...
				$this->version = 1;
				#echo __METHOD__.'::'.__LINE__.'<br>$this->server['.$this->server.']$delegate['.$delegate.']$content['.$content.']';
			}
			...
			#echo __METHOD__.'::'.__LINE__.'<br>';
			#echo __METHOD__.'::'.__LINE__.'<br>$this->server['.$this->server.']$this->identity['.$this->identity.']$delegate['.$delegate.']';
