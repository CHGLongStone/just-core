...
abstract class LightOpenIDProvider
{
    ...
    {
		if(null !== $this->AccessCard){
			#echo __METHOD__.'::'.__LINE__.'$this->AccessCard<pre>'.var_export($this->AccessCard,true).'</pre>';
			$location .= '&openid.AccessCard='.$this->AccessCard;
		}elseif($_REQUEST["openid_AccessCard"]){
			$location .= '&openid.AccessCard='.$_REQUEST["openid_AccessCard"];
		}
    ...
    function __construct()
    {
		...
		#echo __METHOD__.'::'.__LINE__.'$location ['.$location.']<br>';
    ...
    {
        #echo __METHOD__.'::'.__LINE__.'$this->xrdsLocation ['.$this->xrdsLocation.']<br>';
		if($force) {
			#echo __METHOD__.'::'.__LINE__.'$this->xrdsLocation ['.$this->xrdsLocation.']<br>';
        ...
        if (isset($_GET['xrds'])
			...
			#echo __METHOD__.'::'.__LINE__.'$this->xrdsContent() ['.$this->xrdsContent().']<br>';
    ...
    protected function xrdsContent()
    {
        if(isset($this->data["UUID"])){
			$lines = array(
				...
				'        <LocalID>' . $this->serverLocation . $this->data["UUID"].  '</LocalID>',
				'    </Service>',
				'</XRD>',
				'</xrds:XRDS>'
				);
		...
		echo __METHOD__.'::'.__LINE__.'$this->data<pre>'.var_export($this->data,true).'</pre>';
        return implode("\n", $lines);
    }
    ...
    function server()
    {
		#echo __METHOD__.'::'.__LINE__.'$this->assoc<pre>'.md5(var_export($this->assoc, true)).'</pre>';
		#echo __METHOD__.'::'.__LINE__.'$this->data<pre>'.md5(var_export($this->data, true)).'</pre>';
		 #echo __METHOD__.'::'.__LINE__.'$this->data<pre>'.var_export($this->data,true).'</pre>';
		# echo __METHOD__.'@'.__LINE__.'$this->assoc<pre>'.var_export($this->assoc,true).'</pre>';
		#echo __METHOD__.'@'.__LINE__.'$this->data<pre>'.var_export($this->data,true).'</pre>';
        ...
        #
		#echo __METHOD__.'@'.__LINE__.'$this->data<pre>'.var_export($this->data,true).'</pre>';
        #echo __METHOD__.'@'.__LINE__.'$this<pre>'.var_export($this,true).'</pre>';
		#echo __METHOD__.'@'.__LINE__.'$this->assoc<pre>'.var_export($this->assoc,true).'</pre>';
		#
		#echo __METHOD__.'@'.__LINE__.'$this->data["openid_ns"]<pre>'.var_export($this->data['openid_ns'],true).'</pre>';
		#echo __METHOD__.'@'.__LINE__.'$this->ns<pre>'.var_export($this->ns,true).'</pre>';
        ...
        if (isset($this->data['openid_ns'])
            ...
            #
			#
			#echo __METHOD__.'@'.__LINE__.' openid_mode ['.$this->data['openid_mode'].']</br>';
            ...
            case 'checkid_setup':
               #  echo __METHOD__.'@'.__LINE__.'</br>';
				#echo __METHOD__.'===checkid_setup<br>';
                #echo __METHOD__.'===checkid_setup<br>';
				$this->checkRealm();
				...
				#echo __METHOD__.'::'.__LINE__.'$this->data['.$this->data.']</br>';
				/**
				if(!isset($this->data)){
					echo __METHOD__.'@'.__LINE__.'$this->assoc<pre>'.var_export($this->assoc,true).'</pre>';
					#$this->data = $this->assoc["data"];
				}
				echo __METHOD__.'::'.__LINE__.'$this->data[openid_realm]['.$this->data['openid_realm'].']</br>';
				echo __METHOD__.'::'.__LINE__.'$this->data<pre>'.var_export($this->data,true).'</pre>';
				echo __METHOD__.'::'.__LINE__.'$identity = $this->checkid($this->data["openid_realm"], $attrValues)<pre>'.var_export($this->checkid($this->data['openid_realm'], $attrValues),true).'</pre>';
				
				echo __METHOD__.'::'.__LINE__.'$this->data["openid_identity"]<pre>'.var_export($this->data['openid_identity'],true).'</pre>';
				echo __METHOD__.'::'.__LINE__.'$this->select_id<pre>'.var_export($this->select_id,true).'</pre>';
				*/
				
				
                #if (($identity = $this->checkid($this->data['openid_realm'], $attrValues))ORIG
                if (($identity = $this->checkid($this->data['openid_realm'], $attributes))
                    && ($this->select_id || $identity == $this->data['openid_identity'])
                ) {
					#echo __METHOD__.'::'.__LINE__.'$attrValues<pre>'.var_export($attrValues,true).'</pre>';
					#echo __METHOD__.'::'.__LINE__.'$identity<pre>'.var_export($identity,true).'</pre>';
					#$this->redirect($this->positiveResponse($identity, $attrValues););ORIG
					
                    #$this->positiveResponse($identity, $attrValues);
                    $this->positiveResponse($identity, $attributes);
                } elseif($this->data['openid_mode'] == 'checkid_immediate') {
					#echo __METHOD__.'<b>==============='.__LINE__.' checkid_immediate===============</b><br>';
					#echo __METHOD__.'::'.__LINE__.'$identity<pre>'.var_export($identity,true).'</pre>';
					#echo __METHOD__.'::'.__LINE__.'$attrValues<pre>'.var_export($attrValues,true).'</pre>';
					#echo __METHOD__.'::'.__LINE__.'$this->data<pre>'.var_export($this->data,true).'</pre>';
					#
					#echo __METHOD__.'===<pre>'.var_export($this->getAssoc($this->assoc['handle']),true).'</pre><br>';
					#echo __METHOD__.'::'.__LINE__.'$this->response(array("openid.mode" => "setup_needed"))<pre>'.var_export($this->response(array('openid.mode' => 'setup_needed')),true).'</pre>';
					#
					#$this->redirect($this->response(array('openid.mode' => 'setup_needed'))); //ORIG
					#$this->redirect($this->response(array('openid.mode' => 'checkid_immediate'))); //ORIG
					/////-----------++++++++++++++++------------------+++++++++++++++++++++++++++++++++++==========
					#
					#
					$this->positiveResponse($this->data['openid_identity'], $attributes); //why was this added with invalid args?
					/////-----------++++++++++++++++------------------+++++++++++++++++++++++++++++++++++==========
					#die;
                } else {
					#echo __METHOD__.'==='.__LINE__.'<br>';
					#echo __METHOD__.'==='.var_export($this->assoc,true).'<br>';
                    if(!$this->assoc) {
                        $this->generateAssociation();
                        $this->assoc['private'] = true;
                    }
                    $this->assoc['data'] = $this->data;
					#echo __METHOD__.'@'.__LINE__.' $this->assoc["data"]['.var_export($this->assoc['data'],true).']</br>';
                    $setAssoc = $this->setAssoc($this->assoc['handle'], $this->assoc);
					#echo __METHOD__.'@'.__LINE__.'<hr>$setAssoc<pre>'.var_export($setAssoc,true).'</pre><hr>';
            ...
            case 'check_authentication':
				...
				#
				#echo __METHOD__.'@'.__LINE__.' </br>';
				#echo __METHOD__.'@'.__LINE__.'<hr>$this->checkRealm()<pre>'.var_export($this->checkRealm(),true).'</pre><hr>';
				#echo __METHOD__.'@'.__LINE__.'<hr>$this->verify()<pre>'.var_export($this->verify(),true).'</pre><hr>';
                #xdebug_print_function_stack( '<b>PASSED CALL!</b>' );
                if($this->verify()) {
					echo "ns:$this->ns\nis_valid:true";
    ...
    protected function checkRealm()
    {
        if (!isset($this->data['openid_return_to'], $this->data['openid_realm'])) {
            #echo __METHOD__.'::'.__LINE__.'$this->data["openid_return_to"]['.$this->data['openid_return_to'].'] $this->data["openid_realm"]['.$this->data['openid_realm'].']</br>';
			$this->errorResponse();
        }
        
        $realm = str_replace('\*', '[^/]', preg_quote($this->data['openid_realm']));
		#echo __METHOD__.'::'.__LINE__.'$realm<pre>'.var_export($realm,true).'</pre>';
        if(!preg_match("#^$realm#", $this->data['openid_return_to'])) {
			#echo __METHOD__.'::'.__LINE__.'$alias<pre>'.var_export($alias,true).'</pre>';
			$this->errorResponse();
        }
    }
    ...
    protected function ax()
    {
        ...
        $this->data['ax'] = true;
		#echo __METHOD__.'::'.__LINE__.'$required<pre>'.var_export($required,true).'</pre>';
		#echo __METHOD__.'::'.__LINE__.'$if_available<pre>'.var_export($if_available,true).'</pre>';
		#echo __METHOD__.'::'.__LINE__.'$this->data<pre>'.var_export($this->data,true).'</pre>';
    ...
    protected function verify()
    {
        ...
        if(empty($this->assoc)) {
            #			echo __METHOD__.'::'.__LINE__.'FAIL!!!$this->assoc<pre>'.var_export($this->assoc,true).'</pre>';
			return false;
        }
        ...
        if(empty($this->assoc['private'])) {
            #echo __METHOD__.'::'.__LINE__.'FAIL!!!$this->assoc["private"]<pre>'.var_export($this->assoc["private"],true).'</pre>';
            #		echo __METHOD__.'::'.__LINE__.'FAIL!!!$this->assoc<pre>'.var_export($this->assoc,true).'</pre>';
			return false;
        }
        ...
        if($this->data['openid_response_nonce'] != $_REQUEST['openid_response_nonce']) {
            #
			echo __METHOD__.'::'.__LINE__.'FAIL!!!$this->data["openid_response_nonce"]<pre>'.var_export($this->data["openid_response_nonce"],true).'</pre>';
			#echo __METHOD__.'::'.__LINE__.'FAIL!!!$this->assoc["nonce"]<pre>'.var_export($this->assoc["nonce"],true).'</pre>';
			#echo __METHOD__.'::'.__LINE__.'FAIL!!!$this->assoc<pre>'.md5(var_export($this->assoc, true)).'</pre>';
			
			#echo __METHOD__.'::'.__LINE__.'FAIL!!!$this->data<pre>'.md5(var_export($this->data, true)).'</pre>';
			#echo __METHOD__.'::'.__LINE__.'FAIL!!!$this->data<pre>'.var_export($this->data,true).'</pre>';
			#echo __METHOD__.'@'.__LINE__.'$_REQUEST<pre>'.var_export($_REQUEST,true).'</pre>';
			return false;
        }
		...
		#echo __METHOD__.'::'.__LINE__.'$this->data["openid_signed"]<pre>'.var_export($this->data['openid_signed'],true).'</pre>';
		#echo __METHOD__.'::'.__LINE__.'$this->assoc<pre>'.var_export($this->assoc,true).'</pre>';
        foreach($signed as $field) {
            $name = strtr($field, '.', '_');
            if(!isset($this->data['openid_' . $name])) {
                #
				#echo __METHOD__.'::'.__LINE__.'FAIL!!!$name==['.$name.']';
				return false;
            }
		...
		#echo __METHOD__.'::'.__LINE__.'$sig<pre>'.var_export($sig,true).'</pre>';
        ...
        if ($this->data['openid_sig'] != 
            ...
            /*
			echo __METHOD__.'@'.__LINE__.'$_REQUEST<pre>'.var_export($_REQUEST,true).'</pre>';
			echo __METHOD__.'::'.__LINE__.'$this->data[openid_sig]==['.$this->data['openid_sig'].']<br> ['.base64_encode(hash_hmac($this->assoc['hash'], $sig, $this->assoc['mac'], true)).']';
			echo __METHOD__.'::'.__LINE__.'FAIL!!!$this->assoc[hash]==['.$this->assoc['hash'].']<br>';
			echo __METHOD__.'::'.__LINE__.'FAIL!!!$this->data[openid_sig]==['.base64_decode($this->assoc['openid_sig']).']<br>';
			echo __METHOD__.'::'.__LINE__.'FAIL!!!$this->data[openid_sig]==['.$this->b64dec($this->assoc['openid_sig']).']<br>';
			echo __METHOD__.'::'.__LINE__.'FAIL!!!$this->assoc[mac]==['.$this->assoc['mac'].']<br>';
			echo __METHOD__.'::'.__LINE__.'FAIL!!!$sig==['.$sig.']<br>';
			echo __METHOD__.'::'.__LINE__.'<br>';
			echo __METHOD__.'::'.__LINE__.'FAIL!!!$hash_hmac==['.hash_hmac($this->assoc['hash'], $sig, $this->assoc['mac'], true).']<br>';
			*/
			#exit;
			return false;
        }
    ...
    protected function positiveResponse($identity, $attributes)
    {
        ...
        #echo '$this->data<pre>'.var_export($this->data,true).'</pre>';
		...
		#echo '$this->assoc<pre>'.var_export($this->assoc,true).'</pre>';
        if (isset($this->data['openid_assoc_handle'])
        ...
        # Signing the $params
		#echo __METHOD__.'::'.__LINE__.'$params<pre>'.var_export($this->keyValueForm($params),true).'</pre>';
		#exit;
        $sig = hash_hmac($this->assoc['hash'], $this->keyValueForm($params), $this->assoc['mac'], true);
		#echo __METHOD__.'::'.__LINE__.'$this->assoc["mac"]<pre>'.var_export($this->assoc['mac'],true).'</pre>';
		#
		#echo __METHOD__.'::'.__LINE__.'$sig<pre>'.var_export($sig,true).'</pre>';
		#
		#exit;
		...
		#echo __METHOD__.'::'.__LINE__.'$req<pre>'.var_export($req,true).'</pre>';
		#echo __METHOD__.'::'.__LINE__.'$this<pre>'.var_export($this,true).'</pre>';
		...
		#echo  __METHOD__.'::'.__LINE__.'$params<pre>'.var_export($params,true).'</pre>';
        #exit;
		foreach($params as $name => $value) {
            $req['openid.' . $name] = $value;
        }
        #
		#echo '$req<pre>'.var_export($req,true).'</pre>';
        #####
		$respsonse = $this->response($req);
        #echo __METHOD__.'::'.__LINE__.'$respsonse<pre>'.var_export($respsonse,true).'</pre>';
		$this->redirect($respsonse);
    }
    ...
    protected function responseAttributes($attributes)
    {
        ...
        foreach($attributes as $name => $value) {
			#echo __METHOD__.'::'.__LINE__.'$value['.$value.']<br>';
		...
		#echo __METHOD__.'::'.__LINE__.'$response<pre>'.var_export($response,true).'</pre>';
        return $response;
    }
    ...
    function __call($name, $args)
    {
        ...
        default:
            throw new BadMethodCallException('$name==['.$name.']$args<pre>'.var_export($args, true).'</pre>');
        }
    }
}
